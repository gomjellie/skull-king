...
const SkullKing: React.FC = () => {
  ...
  const calculateTotalTricks = (round: Round): number => {
    return round.playerScores.reduce(
      (total, player) => total + (player.tricks ?? 0),
      0
    );
  };
  ...
  const shouldEnableSpecialCards = (playerScore: PlayerScore) => {
    return (
      playerScore.bid !== null &&
      playerScore.tricks !== null &&
      playerScore.bid !== 0 &&
      playerScore.tricks !== 0 &&
  ...
  const shouldEnableNextRound = (
    rounds: Round[],
    currentRoundIndex: number
  ) => {
    return (
      rounds.length === 0 ||
      (currentRoundIndex === rounds.length - 1 && rounds.length < 10)
    );
  };
  ...
  const isCurrentRoundComplete = (
    ...
    return currentRound.playerScores.every(
      (playerScore) =>
        playerScore.bid !== null &&
        playerScore.tricks !== null &&
        isTrickLimitReached(rounds[currentRoundIndex])
    );
  };
  ...
  const removePlayer = (index: number) => {
    ...
    playerInputRefs.current = playerInputRefs.current.filter(
      (_, i) => i !== index
    );
  };
  ...
  const updateSpecialCard = (playerIndex: number, cardName: string) => {
    ...
    if (card) {
      ...
      player.specialCards[cardName] = cycleValue(
  ...
  const getSortedPlayerScores = () => {
    return players
      .map(
        (player, index): SortedPlayerScore => ({
          name: player,
    ...
    const rounds: Round[] = compressedRounds.map((round: any) => ({
      ...
      playerScores: round[1].map((score: any, index: number) => {
        ...
        const playerScore: PlayerScore = {
          ...
          bid: score[0],
          tricks: score[1],
          specialCards: Object.fromEntries(
            specialCardNames.map((name, i) => [name, specialCards[i]])
          ),
          score: 0,
        };
    ...
    <div className="p-2 max-w-sm md:max-w-screen-md lg:max-w-screen-lg mx-auto">
              ...
              <div key={playerIndex} className="mb-2 p-2 border rounded">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="font-bold">
                    Score: {playerScore.score} / {calculateTotal(playerIndex)}
                  </span>
                </div>
                    ...
                    {new Array(currentRoundIndex + 2).fill(0).map((_, i) => (
                      <label className="border border-gray-300 w-full stretch rounded-md has-[:checked]:bg-black has-[:checked]:text-white  hover:bg-gray-500">
                        <input
                          className="hidden"
                          type="radio"
                          name={`${playerIndex}-bid`}
                          value={i}
                          checked={playerScore.bid === i}
                          onChange={() =>
                    ...
                    {new Array(currentRoundIndex + 2).fill(0).map((_, i) => (
                      <label className="border border-gray-300 w-full stretch rounded-md has-[:checked]:bg-black has-[:checked]:text-white  hover:bg-gray-500">
                        <input
                          className="hidden"
                          type="radio"
                          name={`${playerIndex}-tricks`}
                          value={i}
                          checked={playerScore.tricks === i}
                          onChange={() =>
                  ...
                  {specialCards.map((card) => (
                    ...
                    >
                      ...
                      <span className="font-bold ml-1 text-sm">
                        {playerScore.specialCards[card.name]}
                      </span>
                    </Button>
                  ))}
                </div>
      ...
      {shouldEnableNextRound(rounds, currentRoundIndex) && (
        <Button
          ...
          className="w-full"
          disabled={!isCurrentRoundComplete(rounds, currentRoundIndex)}
        >
      ...
      <AlertDialog>
        ...
        <AlertDialogContent className="max-w-[300px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action will reset all players and scores. This cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        {showQRCode && (
          <div className="mt-4 flex flex-col items-center">
            ...
            <p className="mt-2 text-sm">
              Link copied to clipboard! Or scan this QR code
            </p>
          </div>
        )}
        {!showQRCode && (
